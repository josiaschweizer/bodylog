...
export default function MedicationEntryFields({
  value,
  medications,
  onChange,
}: MedicationEntryFieldsProps) {
      ...
      {medications.length ? (
        <>
          ...
          <div className="mt-4 grid grid-cols-2 gap-3">
            <label>
              ...
              <input
                ...
                step="0.001"
                value={value.dose}
                onChange={(event) => onChange({ ...value, dose: event.target.value })}
            ...
            <label>
              ...
              <input
                ...
                onChange={(event) => onChange({ ...value, doseUnit: event.target.value })}
                placeholder="z. B. mg"
                className="w-full rounded-xl border border-dusty-taupe-300 bg-white px-3 py-3 outline-none focus:border-chocolate-plum-500 focus:ring-4 focus:ring-chocolate-plum-100"
              />
            </label>
          ...
          <label className="mt-4 flex items-center gap-3 rounded-xl bg-khaki-beige-50 px-4 py-3 text-sm font-medium text-ash-brown-800">
            <input
              type="checkbox"
              checked={value.takenAsNeeded}
              onChange={(event) => onChange({ ...value, takenAsNeeded: event.target.checked })}
