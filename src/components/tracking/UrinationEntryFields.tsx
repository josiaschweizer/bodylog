  ...
  return (
    <fieldset className="rounded-2xl border border-dusty-taupe-200 bg-white p-4 sm:p-5">
      <legend className="px-2 text-sm font-bold text-chocolate-plum-800">
        Details zum Wasserlassen
      </legend>
      <div className="grid gap-4 sm:grid-cols-2">
        <label>
          <span className="mb-2 block text-sm font-semibold">Menge</span>
          <select
            value={value.amount}
            onChange={(event) => update({ amount: event.target.value as UrineAmount | '' })}
            className="w-full rounded-xl border border-dusty-taupe-300 bg-white px-3 py-3 outline-none"
          >
        ...
        <label>
          <span className="mb-2 block text-sm font-semibold">Farbe</span>
          <input
            list="urine-colors"
            value={value.color}
            ...
            className="w-full rounded-xl border border-dusty-taupe-300 bg-white px-3 py-3 outline-none"
          />
          <datalist id="urine-colors">
            <option value="Klar" />
            <option value="Hellgelb" />
            <option value="Gelb" />
            <option value="Dunkelgelb" />
            <option value="Orange" />
            <option value="Rötlich" />
          </datalist>
        </label>
      ...
      <div className="mt-4 grid gap-2 sm:grid-cols-2">
        <label className="flex items-center gap-3 rounded-xl border border-dusty-taupe-200 px-3 py-3 text-sm font-medium">
          <input
            type="checkbox"
            checked={value.burning}
            onChange={(event) => update({ burning: event.target.checked })}
        ...
        <label className="flex items-center gap-3 rounded-xl border border-dusty-taupe-200 px-3 py-3 text-sm font-medium">
          <input
            type="checkbox"
            checked={value.nighttime}
            onChange={(event) => update({ nighttime: event.target.checked })}
            className="size-4 accent-chocolate-plum-700"
          />
          Nachts
        </label>
