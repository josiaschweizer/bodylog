...
const navigation = [
  ...
  { to: '/analytics', label: 'Auswertung', icon: ChartNoAxesCombined, end: false },
...
export default function AppLayout() {
  ...
  return (
    <div className="min-h-svh bg-khaki-beige-50 text-ash-brown-900 lg:grid lg:grid-cols-[16rem_minmax(0,1fr)] print:block print:bg-white">
      <aside className="fixed inset-y-0 left-0 z-30 hidden w-64 flex-col border-r border-dusty-taupe-200 bg-white px-5 py-6 lg:flex print:hidden">
      ...
      <div className="lg:col-start-2 print:block">
        <header className="sticky top-0 z-20 flex h-[calc(4rem+env(safe-area-inset-top))] items-end border-b border-dusty-taupe-200 bg-khaki-beige-50/90 px-5 pb-5 pt-[env(safe-area-inset-top)] backdrop-blur lg:hidden print:hidden">
      ...
      <button
        type="button"
        onClick={openNewEntry}
        className="fixed bottom-[calc(6.25rem+env(safe-area-inset-bottom))] right-5 z-30 flex min-h-13 items-center gap-2 rounded-full bg-chocolate-plum-800 px-6 py-3.5 text-base font-semibold text-white shadow-xl shadow-chocolate-plum-950/25 transition active:scale-95 lg:hidden print:hidden"
      >
      ...
      <nav
        className="fixed inset-x-0 bottom-0 z-20 grid h-[calc(5.25rem+env(safe-area-inset-bottom))] grid-cols-3 border-t border-dusty-taupe-200 bg-white pb-[env(safe-area-inset-bottom)] shadow-[0_-8px_24px_rgba(29,25,22,0.06)] lg:hidden print:hidden"
